...
with st.sidebar.expander("📍 Ubicación y Edificio", expanded=True):
    ...
    suelo = st.selectbox("Tipo de Suelo", ["A", "B", "C", "D", "E"], index=1, help="Según NCh 433")
    cat_edif = st.selectbox("Categoría de Ocupación", ["I", "II", "III", "IV"], index=1)
...
with st.sidebar.expander("🔩 Propiedades del Componente", expanded=True):
    ...
    amp_vert = st.checkbox("Amplificación Vertical (x2.5)", value=True, help="Según cláusula 6.1 (pág. 14)")

# =================================================================
# 3. MOTOR DE CÁLCULO (LÓGICA NCh 3357)
# =================================================================

# 1. Parámetros de aceleración (Tablas 2 y 3)
z_map = {1: 0.50, 2: 0.75, 3: 1.00}
Z_val = z_map[zona]
# Parámetro alpha_A * A (cm/s2) 
suelo_map = {"A": 977, "B": 1101, "C": 1144, "D": 1455, "E": 1576}
alpha_A_A = suelo_map[suelo] * Z_val
alpha_A_A_g = alpha_A_A / 980.665 # g

# 2. Factor de Importancia Ip (Cláusula 5.1) [cite: 787]
Ip = 1.5 if cat_edif in ["III", "IV"] else 1.0

# 3. Aceleración Horizontal ah (Ecuación 1) [cite: 861]
rel_h = z_nivel / h_total
ah_calc = (0.4 * ap * alpha_A_A_g) / (Rp / Ip) * (1 + 2 * rel_h)

# Límites normativos (Ecuaciones 2 y 3) [cite: 862]
ah_min = 0.3 * alpha_A_A_g * Ip
ah_max = 1.6 * alpha_A_A_g * Ip
ah_final = max(ah_min, min(ah_calc, ah_max))

# 4. Aceleración Vertical av (Cláusula 6.1) 
av_base = 0.24 * alpha_A_A_g * Ip
av_final = av_base * 2.5 if amp_vert else av_base
...
def generar_pdf_sismo():
    ...
    pdf.cell(0, 8, f" Zona: {zona} | Suelo: {suelo} | Categoria: {cat_edif} (Ip={Ip})", ln=True)
    pdf.cell(0, 8, f" Altura h: {h_total} m | Nivel z: {z_nivel} m | Wp: {peso_comp} kgf", ln=True)
    pdf.ln(5)
    ...
    pdf.cell(0, 10, f" Fuerza Vertical de Diseno (Fpv): {Fp_v:.2f} kgf", ln=True)
...
<div class="classification-box">
    ...
    Categoría Edificio: {cat_edif} | Factor Ip: {Ip:.1f} | Suelo: {suelo}<br>
...
with col_g1:
    ...
    ax_h.set_xlabel("Altura z (m)"); ax_h.set_ylabel("Aceleración (g)"); ax_h.grid(True, alpha=0.3); ax_h.legend()
    st.pyplot(fig_h)

with col_g2:
    st.subheader("📈 Sensibilidad Vertical av")
    # El sismo vertical es constante en toda la altura 
    fig_v, ax_v = plt.subplots(figsize=(8, 6))
    ax_v.axhline(av_final, color='#d9534f', lw=2.5, label=f'av ({av_final:.3f}g)')
    if amp_vert:
        ax_v.axhline(av_base, color='gray', ls=':', label='av base (sin amp. 2.5)')
    ax_v.scatter([z_nivel], [av_final], color='black', s=100, zorder=5)
    ax_v.set_xlabel("Altura z (m)"); ax_v.set_ylabel("Aceleración (g)"); ax_v.set_ylim(0, max(av_final*1.5, 0.5))
